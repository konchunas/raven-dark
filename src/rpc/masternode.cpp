...
UniValue masternodecoldcreate(const UniValue &params, bool fHelp) 
{
    if (fHelp)
        throw std::runtime_error(
                "masternodecoldcreate \"command\"...\n"
                        "Set of commands to create masternode broadcast totally offline\n"
                        "\nArguments:\n"
                        "1. ip:port         (string, required) External ip and port of masternode\n"
                        "2. masternodekey   (string, required) Masternode private key generated by genkey.\n"
                        "3. address         (string, required) Masternode address where collateral tx was sent to.\n"
                        "4. collateral_txid (string, required) Tx id of transaction where collateral unspent output is.\n"
                        "5. output_index    (numeric, required) Index of output where collateral sum stored.\n"
                        "6. best_block      (string, required) Best block hash in the network at a time.\n"
    ...
    auto output_index = params[4].get_int();
    ...
    bool fResult = CMasternodeBroadcast::ColdCreate(ip, node_key,
